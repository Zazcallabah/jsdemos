...
namespace Noise.Generators
{
	public class BenchmarkNoise
	{
		...
		public BenchmarkNoise( int seed )
		{
		...
		public double Noise( double x, double y, double z )
		{
			...
			double fx0 = x - ix;
			double fx1 = fx0 - 1;
			double wx = Weighted( fx0 );

			int iy = (int) Math.Floor( y );
			double fy0 = y - iy;
			double fy1 = fy0 - 1;
			double wy = Weighted( fy0 );

			int iz = (int) Math.Floor( z );
			double fz0 = z - iz;
			double fz1 = fz0 - 1;
			double wz = Weighted( fz0 );

			double vx0 = Lattice( ix, iy, iz, fx0, fy0, fz0 );
			double vx1 = Lattice( ix + 1, iy, iz, fx1, fy0, fz0 );
			double vy0 = Linear( wx, vx0, vx1 );

			vx0 = Lattice( ix, iy + 1, iz, fx0, fy1, fz0 );
			vx1 = Lattice( ix + 1, iy + 1, iz, fx1, fy1, fz0 );
			double vy1 = Linear( wx, vx0, vx1 );

			double vz0 = Linear( wy, vy0, vy1 );

			vx0 = Lattice( ix, iy, iz + 1, fx0, fy0, fz1 );
			vx1 = Lattice( ix + 1, iy, iz + 1, fx1, fy0, fz1 );
			vy0 = Linear( wx, vx0, vx1 );

			vx0 = Lattice( ix, iy + 1, iz + 1, fx0, fy1, fz1 );
			vx1 = Lattice( ix + 1, iy + 1, iz + 1, fx1, fy1, fz1 );
			vy1 = Linear( wx, vx0, vx1 );

			double vz1 = Linear( wy, vy0, vy1 );
			return Linear( wz, vz0, vz1 );
		}
		...
		static double Weighted( double x )
		{
			return ( 3 - 2 * x ) * x * x;
		}
		int Permutate( int x )
